...
function generateHTMLContent(itemName, itemPrice) {
    return `
        <div class="card m-1 card-menu">
            <img class="card-img-top card-image" src="images/butted-shrimp.jpg" alt="Food Image">
            <div class="card-body d-flex justify-content-between flex-column">
            <div>
                <h5 class="card-title">${itemName}</h5>
                <p class="card-text">Php ${itemPrice}</p>
                </div>
                <div class="d-grid gap-2 col-6 mx-auto">
                    <button class="btn custom-btn-bg" type="button" data-bs-toggle="modal" data-bs-target="#orderModal" data-item-name="${itemName}" data-item-price="${itemPrice}">Order</button>
                </div>
            </div>
        </div>`;
}
        ...
        const orderHTML = `
            <div class="row mt-1 mb-1 row-items" id="${rowId}">
                ...
                <div class="col-2 list-item p-0">
                    <button class="btn-x btn btn-sm btn-outline-danger" onclick="openConfirmationModal('${dish}', ${total}, ${qty}, '${rowId}')">X</button>
                </div>
            </div>
        `;
